...
try:
    ...
    pass
print("Importing target module: itertools", file=stderr)
try:
    import itertools
except ImportError as _fusil_import_error:
    print("FUSIL: target module itertools not importable (skipping):", repr(_fusil_import_error), file=stderr)
    raise SystemExit(0)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"itertools.{method_name}()" if obj_to_call is itertools else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, itertools, func_name_str, *arguments, verbose=verbose)

fuzz_target_module = itertools
...
# --- TSan concurrency-stress region (--tsan) ---
...
print('[TSAN] provenance module=itertools shared=target-objects(0obj+3cls+0plugin+module) weird=0 args=list,dict,set,bytearray iterators=8+6ext iter_off=3 roles=r/w/both ops=a-i workers=4 iters=200 funcs=0', file=stderr)
print('[TSAN-MANIFEST] {"ext_iterators": 6, "func_count": 0, "iter_off": 3, "iterators": ["str", "bytes", "list", "tuple", "dict", "range", "itertools.count", "struct.iter_unpack"], "iters": 200, "kind": "tsan-provenance", "module": "itertools", "mutate_callables": false, "mutate_state": false, "mutate_types": false, "ops": ["a:read-churn", "b:method", "c:module-func", "d:attr-dict", "e:weakref", "f:container-mutate", "g:gc", "h:shared-iter", "i:read-while-mutate"], "plugin_factories": [], "roles": {"0": "writer", "1": "reader", "2": "both"}, "shared_args": ["list", "dict", "set", "bytearray"], "shared_classes": ["_tee", "zip_longest", "starmap"], "shared_kind": "target-objects", "shared_objects": [], "shared_objects_only": false, "v": 1, "weird_subclasses": [], "workers_per_obj": 4}', file=stderr)
...
_MUTATE_STATE = False
_MUT_REG = globals().get("_FUSIL_STATEFUL_MUTATORS", {})
_TSAN_OBJ_ONLY = False
_tsan_call_args = ["x", "fusil", 1, 2, 0, -1, 1.5, True, ["a", "b"], ("a",), {"a": 1}, b"x", None]
_MUTATE_TYPES = False
...
_tsan_obj_factories = []
_tsan_obj_factories.append(lambda: getattr(fuzz_target_module, '_tee')())
_tsan_obj_factories.append(lambda: getattr(fuzz_target_module, '_tee')(IndexBomb(), 9.16))
_tsan_obj_factories.append(lambda: getattr(fuzz_target_module, 'zip_longest')())
_tsan_obj_factories.append(lambda: getattr(fuzz_target_module, 'zip_longest')(tuple[weird_classes['weird_deque']], '/tmp/fusil-fixtures/fusil_fixture.txt'))
_tsan_obj_factories.append(lambda: getattr(fuzz_target_module, 'starmap')())
_tsan_obj_factories.append(lambda: getattr(fuzz_target_module, 'starmap')(r"nAirBFuG.PAoICd.", "\x00"))
_TSAN_MAX_SHARED = 3
_tsan_shared = []
...
try:
    ...
    pass
_tsan_iter_factories.append(lambda: iter(getattr(fuzz_target_module, '_tee')()))
_tsan_iter_factories.append(lambda: iter(getattr(fuzz_target_module, '_tee')(IndexBomb(), 9.16)))
_tsan_iter_factories.append(lambda: iter(getattr(fuzz_target_module, 'zip_longest')()))
_tsan_iter_factories.append(lambda: iter(getattr(fuzz_target_module, 'zip_longest')(tuple[weird_classes['weird_deque']], '/tmp/fusil-fixtures/fusil_fixture.txt')))
_tsan_iter_factories.append(lambda: iter(getattr(fuzz_target_module, 'starmap')()))
_tsan_iter_factories.append(lambda: iter(getattr(fuzz_target_module, 'starmap')(r"nAirBFuG.PAoICd.", "\x00")))
_tsan_iters = []
_tsan_iter_ok = []
...
for _t in _tsan_threads:
    _t.join()
print("[TSAN] concurrency-stress region complete", file=stderr)
