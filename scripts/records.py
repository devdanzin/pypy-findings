...
RECORDS = [
    ...
    dict(
        ...
        analysis="`W_ZipLongest._fetch` stores None into `self.iterators_w[i]` when sub-iterator i "
                 ...
                 "on it with no None check (line 681), which in translated RPython is a dereference of a "
                 "null pointer. CONFIRMED UNDER GDB rather than inferred (gdb.txt): the fault is in "
                 "`pypy_g_W_ZipLongest_iterator_greenkey+47` on `mov (%rdi),%eax` with `rdi = 0x0` -- the "
                 "typeid load at the head of the object -- called directly from "
                 "`pypy_g.do_extend_from_iterable`, exactly the path the source reading predicts. "
                 "`iterator_greenkey` is consulted by the space-level BULK UNPACK path -- "
